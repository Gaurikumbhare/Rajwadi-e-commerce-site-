...
// Supabase completely removed; using SQLite backend.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fetch products from our new backend API instead of local static array
  fetch("http://localhost:3000/api/products")
    ...
    .catch(err => {
      ...
      // Fallback to local products array if backend fails
      renderCatalog(window.products || []);
    });
  ...
  // Check for mock session in localStorage (Fallback since we use SQLite API)
...
async function submitOrder(event) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("Failed to place order via backend API");
    
    console.log("Order saved to database successfully.");
  } catch (err) {
    console.warn("Could not write order to database:", err.message);
    showToast("Order simulated locally (backend not running).");
...
async function fetchUserOrders() {
  ...
  try {
    ...
    console.error("Error loading order history from backend:", err);
