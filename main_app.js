...
// App State & Data
const SUPABASE_URL = 'https://jyywiywbcaxuncyradpv.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp5eXdpeXdiY2F4dW5jeXJhZHB2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAzODQ3NjIsImV4cCI6MjA5NTk2MDc2Mn0.AMLbYZq2wQl514shlh6nFcY2pzLSvtlpsEbZ_f1JGNQ';
...
let currentSessionUser = null; // Currently logged in user (mocked)
    ...
    supabaseClient.from('products').select('*')
      .then(({ data, error }) => {
        if (error || !data || data.length === 0) {
          console.error("Failed to fetch products from Supabase or table is empty:", error);
          ...
          const normalizedProducts = data.map(p => ({
              ...
              stitchingOptions: p.stitching_options,
              inStock: p.in_stock,
              tags: p.tags,
              badge: p.badge,
...
function showView(viewId, skipCatalogReset) {
  ...
  renderView(viewId, skipCatalogReset);
}


// Render the target view section
// skipCatalogReset: if true, does NOT reset filters/products (caller will do it)
function renderView(viewId, skipCatalogReset) {
  // Hide all sections
  ...
  if (viewId === "catalog" && !skipCatalogReset) {
    resetSidebarFilters();
    renderCatalog(window.products || []);
    const titleEl = document.getElementById("catalogTitle");
    if (titleEl) titleEl.innerText = "Ethnic Wear Collections";
  }
}
...
function filterByCategory(categoryName) {
  ...
  // Set catalog title
  const titleEl = document.getElementById("catalogTitle");
  if (titleEl) titleEl.innerText = `${categoryName} Collection`;
}

function filterByOccasion(occasionName) {
  // Switch to catalog WITHOUT resetting (we handle it ourselves)
  showView('catalog', true);
...
function filterByStore(storeType) {
  if (storeType === 'Kids') {
    showToast("Kids Royal Collection is coming soon! Pre-register to get notified.", false);
    return;
  }
  if (storeType === 'Home') {
    showToast("Rajwadi Home Decor & Furnishings Collection is coming soon!", false);
    return;
  }

  // Switch to catalog view WITHOUT resetting filters
  showView('catalog', true);
  resetSidebarFilters();
  ...
  if (storeType === 'Women') {
    // Women category contains Saree, Lehenga, Salwar Kameez
    const checks = document.querySelectorAll(".category-filter-check");
    checks.forEach(check => {
      if (check.value === 'Saree' || check.value === 'Lehenga' || check.value === 'Salwar Kameez') {
        check.checked = true;
      }
    });
    applyFilters();
    if (titleEl) titleEl.innerText = "Women's Royal Collection";
  } else if (storeType === 'Men') {
    // Men category contains Sherwani
    const checks = document.querySelectorAll(".category-filter-check");
    checks.forEach(check => {
      if (check.value === 'Sherwani') {
        check.checked = true;
      }
    });
    applyFilters();
    if (titleEl) titleEl.innerText = "Grooms Wear Collection";
  } else if (storeType === 'Luxe') {
    // Luxe Edit: Filter by price range over $300
    const radioOver300 = document.querySelector('input[name="price-filter"][value="over300"]');
    if (radioOver300) radioOver300.checked = true;
    applyFilters();
    if (titleEl) titleEl.innerText = "Luxe Edit - Premium Collections";
  } else if (storeType === 'Gems') {
    // Hidden Gems: Filter by colors Red and Gold
    const redCheck = document.querySelector('.color-filter-check[value="Red"]');
    const goldCheck = document.querySelector('.color-filter-check[value="Gold"]');
    if (redCheck) redCheck.checked = true;
    if (goldCheck) goldCheck.checked = true;
    applyFilters();
    if (titleEl) titleEl.innerText = "Hidden Gems (Ruby & Gold Collection)";
  } else if (storeType === 'Global') {
    // Global Store: Reset all filters to show everything
    resetSidebarFilters();
    applyFilters();
    if (titleEl) titleEl.innerText = "Global Store Collections";
  } else if (storeType === 'Revolve') {
    // Revolve: Georgette fabric
    const georgetteCheck = document.querySelector('.fabric-filter-check[value="Georgette"]');
    if (georgetteCheck) georgetteCheck.checked = true;
    applyFilters();
    if (titleEl) titleEl.innerText = "Revolve - Fusion Georgette Collections";
  }
}
...
function triggerSearch() {
  ...
  // Switch to catalog WITHOUT resetting (we handle rendering ourselves)
  showView('catalog', true);

  // Hide search overlay panel
  toggleSearchOverlay(false);

  // Reset sidebar filters before searching
  resetSidebarFilters();
...
function applyFilters() {
  ...
  const selectedPriceRange = priceChecked ? priceChecked.value : 'all';
  const sortEl = document.getElementById("sortSelect");
  const activeSort = sortEl ? sortEl.value : 'default';
...
function renderCatalog(items) {
  ...
  items.forEach(product => {
    ...
    card.innerHTML = `
      ${tagHtml}
      <div class="product-img-wrapper" style="position: relative; cursor: pointer;">
        <button onclick="event.stopPropagation(); toggleWishlist('${product.id}')" style="position: absolute; top: 10px; right: 10px; background: white; border: none; border-radius: 50%; width: 34px; height: 34px; display: flex; align-items: center; justify-content: center; z-index: 10; box-shadow: 0 2px 8px rgba(0,0,0,0.15); cursor: pointer; transition: all 0.2s;">
          <i class="${wishlist.includes(product.id) ? 'fa-solid' : 'fa-regular'} fa-heart" style="color: ${wishlist.includes(product.id) ? 'var(--color-maroon)' : '#555'}; font-size: 16px;"></i>
        </button>
        <div onclick="showProductDetails('${product.id}')" style="width: 100%; height: 100%;">
...
function toggleWishlist(productId) {
  ...
  // Re-render catalog and details if active
  const activeView = document.querySelector('.view-section.active');
  if (activeView && activeView.id === 'catalog-view') {
    applyFilters();
...
function updateWishlistUI() {
  ...
  if (wishlist.length === 0) {
    list.innerHTML = `
      <div class="cart-empty-message">
        ...
        <button class="btn-gold" style="margin-top: 20px;" onclick="toggleWishlistSidebar(false); showView('catalog');">Browse Catalog</button>
      </div>
    `;
    return;
  }
  ...
  wishlist.forEach(productId => {
    ...
    row.innerHTML = `
      <div class="cart-item-img" onclick="toggleWishlistSidebar(false); showProductDetails('${product.id}')" style="cursor: pointer;">
        <img src="${product.image}" alt="${product.name}">
      </div>
      <div class="cart-item-details">
        <h4 class="cart-item-title" onclick="toggleWishlistSidebar(false); showProductDetails('${product.id}')" style="cursor: pointer;">${product.name}</h4>
        <div class="cart-item-price-qty">
          <span class="cart-item-price">₹${product.price.toFixed(2)}</span>
        </div>
      </div>
      <button class="cart-remove-btn" onclick="toggleWishlist('${product.id}')" title="Remove"><i class="fa-solid fa-xmark"></i></button>
    `;
...
async function submitOrder(event) {
  ...
  if (currentUser) {
    ...
    if (error) {
      console.warn("Could not write order to database:", error.message);
      showToast("Order could not be saved to backend, simulating locally.", true);
    }
  } else {
    showToast("Guest order simulated locally.", false);
  }
...
/* ==========================================================================
   CONSULTATION FORM SUBMISSION (FORMSUBMIT API)
   ========================================================================== */
function submitConsultation(event) {
  ...
  const nameInput = form.querySelector('input[placeholder="Your Excellency"]');
  const emailInput = form.querySelector('input[placeholder="contact@example.com"]');
  const locationSelect = form.querySelector('select');
  const dateInput = form.querySelector('input[type="date"]');
  const requirementsTextarea = form.querySelector('textarea');
  ...
  fetch("https://formsubmit.co/ajax/gauri.kumbhare.try1@gmail.com", {
    method: "POST",
    headers: { 
    ...
    body: JSON.stringify({
        _subject: `Bespoke Consultation Request - ${name}`,
        Name: name,
        Email: email,
  ...
  .then(data => {
      showToast('Your consultation request has been successfully sent to the admin.');
      form.reset();
  })
  .catch(error => {
      showToast('Error sending consultation request. Please try again.', true);
  })
  .finally(() => {
...
async function fetchUserOrders() {
  ...
  try {
    ...
    console.error("Error loading order history from Supabase:", err);
    ordersList.innerHTML = `<p class="no-orders-message" style="color: #e74c3c;">Failed to load order history from backend server.</p>`;
  }
}
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        showToast("Error connecting to backend server. Make sure node server.js is running.", true);
      }
    });
  }
});

// Store Carousel Scroll Logic
function scrollStores(direction) {
  const container = document.getElementById("storesScrollContainer");
  if (container) {
    // scroll amount: roughly 2 items width
